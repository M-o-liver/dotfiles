...
6. **Fonts are not in git** (JetBrainsMono Nerd Font, ~220MB). They come from `install-nerd-font`. If terminals show tofu boxes (missing glyphs), re-run that script before debugging anything else.
...
| gnome-terminal (primary terminal) | Fedora repos (dnf) | The RHEL-supported VTE terminal; sets `TERM=xterm-256color` so SSH into minimal boxes just works. Themed Catppuccin Mocha by the `term-theme` script (dconf; stow pkg `scripts/`). Replaced Ghostty 2026-07-23. |
...
- **Terminal switched Ghostty → gnome-terminal (2026-07-23):** the user chose the RHEL-supported VTE terminal over Ghostty, mainly because Ghostty sets `TERM=xterm-ghostty` and SSHing into freshly-installed minimal boxes (no `xterm-ghostty` terminfo there) broke keys/colors; gnome-terminal's `xterm-256color` is present everywhere. Ghostty and its COPR (`scottames/ghostty`) were removed; its stow package is deleted from the repo (recoverable via git history). Ptyxis (Fedora's default terminal) was also removed — nothing depended on either (`dnf repoquery --whatrequires` was empty for both). The whole point stands: shell + tools are decoupled from the emulator, so nothing was lost. Terminal colors now come from `term-theme` (dconf, Catppuccin Mocha), NOT gnome-nord. **Login shell is now zsh** (`chsh -s /usr/bin/zsh`); previously bash, and Ghostty had masked that by forcing `command = zsh` in its config — which is why gnome-terminal first opened bash until the shell was actually changed.
...
Terminal: gnome-terminal (Catppuccin Mocha via `term-theme`; replaced
Ghostty 2026-07-23) · File manager: yazi (terminal) + Nautilus (GUI) ·
Shell: zsh + starship (login shell is zsh via chsh). The shell tooling was
explicitly kept out of the 2026-07-23 desktop revert — "default GNOME"
meant the desktop, not the terminal tooling.
